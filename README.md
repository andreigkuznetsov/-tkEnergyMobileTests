...
- <b>Browserstack</b> используется как платформа для удаленного запуска тестов. 

- <b>Selenoid</b> используется для запуска браузеров в контейнерах <b>Docker</b>.

- При помощи <b>Appium</b> запускаются автотесты локально (на реальном устройстве и в эмуляторе) 
